...
class ScaffoldWorkerRepoTests(unittest.TestCase):
    ...
    def test_scaffold_renders_worker_template(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn('"check": "npm ci && npm test && npm run typecheck && npm run deploy:dry-run"', package_json)
            self.assertIn('"vibe": "npm run check && npm run dev"', package_json)
    ...
    def test_next_steps_suggests_repo_create_when_skipped(self):
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            self.assertIn("npm run vibe", output)
